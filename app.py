...
<script>
let current, active=0, showcaseData; const typeColors={"裁判書":"#cfe2cc","被告":"#cfe2cc","被害主體":"#cfe2cc","犯罪工具":"#00a000","犯罪原因":"#ffff8a","涉案行為":"#ffff8a","後果評估":"#ff7200"};
...
function buildCasePayload(graphId){const query=showcaseData.queries[graphId],retrieval=showcaseData.retrievals[graphId],showcase=showcaseData.showcase[graphId];if(!query||!retrieval||!showcase)return null;const matches=(retrieval.top_5_retrieved||[]).map((item,index)=>{const reference=showcase.references[index],history=showcaseData.histories[item.similar_graph_id];return history&&reference?{rank:index+1,score:item.score,case:history,graph:showcaseData.graphs[item.similar_graph_id]||{nodes:[],edges:[]},drawio_svg:'assets/drawio/'+reference.svg}:null}).filter(Boolean);return {query,query_graph:showcaseData.graphs[graphId]||{nodes:[],edges:[]},matches,llm:showcaseData.llm[graphId],drawio_query_svg:'assets/drawio/'+showcase.query_svg};}
async function init(){showcaseData=await (await fetch('./data/showcase_data.json')).json();const list=showcaseData.query_list;const sel=document.querySelector('#querySelect');sel.innerHTML=list.map(q=>`<option value="${esc(q.graph_id)}">${esc(q.charge)}｜${esc(q.id)}</option>`).join('');sel.addEventListener('change',()=>load(sel.value));load(list[0].graph_id);}
function load(id){document.querySelector('#loading').hidden=false;document.querySelector('#content').hidden=true;current=buildCasePayload(id);active=0;if(!current)throw new Error('找不到案件');document.querySelector('#content').hidden=false;render();document.querySelector('#loading').hidden=true;}
...
function render(){ const q=current.query,m=current.matches[active], llm=current.llm, content=document.querySelector('#content'); content.innerHTML=`
 ...
 <section class="card"><h2>步驟 3｜具案例依據的 LLM 推理</h2><p class="hint">LLM 以本案事實為小前提，結合法規知識與上述五件檢索案例作為大前提，產出罪名、可能適用條文與推理結論。</p>${llm?`<div class="grid"><div><strong>建議罪名</strong><p>${esc(llm.prediction.predicted_charge||'—')}</p></div><div><strong>可能適用條文</strong><p>${esc(llm.prediction.predicted_article||'—')}</p></div></div><details><summary>查看完整 LLM 推理報告</summary><pre class="reasoning">${esc(llm.reasoning||'此案件未提供完整推理報告。')}</pre></details>`:'<p class="hint">此案件沒有對應的批次 LLM 推理結果，因此不顯示模型結論。</p>'}</section>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802
        ...
        if url.path == "/data/showcase_data.json" or url.path.startswith("/assets/"):
            asset = (BASE_DIR / url.path.lstrip("/")).resolve()
            if BASE_DIR.resolve() not in asset.parents or not asset.is_file():
